...
namespace app\widgets\redactor\actions;

use app\widgets\redactor\Widget;
use yii\base\Action;
...
/**
 * Class UploadAction
 * @package app\widgets\redactor\actions
 *
 ...
 *     return [
 *         'upload-image' => [
 *             'class' => 'app\widgets\redactor\actions\UploadAction',
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($model->hasErrors()) {
                ...
                if ($model->file->saveAs($this->path . $model->file->name)) {
                    $result = ['file' => $this->url . $model->file->name];
                    if ($this->uploadOnlyImage !== true) {
                        $result['file'] = $model->file->name;
                    }
                } else {
                    $result = [
                        'error' => Yii::t('admin', 'ERROR_CAN_NOT_UPLOAD_FILE')
                    ];
                }
            }
